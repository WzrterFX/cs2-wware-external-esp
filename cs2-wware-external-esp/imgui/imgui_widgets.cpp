...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = GetFrameHeight() * 0.75f;
    ImVec2 pos = window->DC.CursorPos;

    const ImVec2 total_size(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f);
    ...
    ImU32 back_col = GetColorU32(ImGuiCol_WindowBg);
    ...
    window->DrawList->AddRectFilled(newRect.Min, newRect.Max, back_col, 16);
    ...
    window->DrawList->AddRectFilled(sRect.Min, sRect.Max, ImColor(240, 240, 240), 16);

    if (*v)
    {
        window->DrawList->AddRectFilled(ImVec2(newRect.Min.x - 1.f, newRect.Min.y - 1.f), ImVec2(newRect.Max.x + .5f, newRect.Max.y + 1.f), check_col, 18);
        ...
        window->DrawList->AddRectFilled(tRect.Min, tRect.Max, ImColor(255, 255, 255), 16);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImVec2 spacing(0.0f, 6.0f);
    window->DC.CursorPos += spacing;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float frame_height = ImMax(label_size.y * 0.5f, GetTextLineHeight() * 0.5f + style.FramePadding.y * 0.1f);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, frame_height));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    const float inner_spacing_for_text = label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f;
    ...
    // Render background of the slider
    ...
    if (*(float*)p_min < *(float*)p_max)
        fill_value = ImClamp((*(float*)p_data - *(float*)p_min) / (*(float*)p_max - *(float*)p_min), 0.0f, 1.0f);

    // Make the grab area thinner
    const float grab_width = ImMin(style.GrabMinSize * 0.5f, frame_bb.GetWidth());
    // Center the grab handle within the slider range
    float grab_center_x = ImLerp(frame_bb.Min.x + grab_width * 0.5f, frame_bb.Max.x - grab_width * 0.5f, fill_value);
    grab_bb.Min.x = grab_center_x - grab_width * 0.5f;
    grab_bb.Max.x = grab_center_x + grab_width * 0.5f;

    // Align fill and grab indicators properly
    const ImVec2 fill_rect_min = ImVec2(frame_bb.Min.x, grab_bb.Min.y);
    const ImVec2 fill_rect_max = ImVec2(grab_bb.Max.x, grab_bb.Max.y);
    window->DrawList->AddRectFilled(fill_rect_min, fill_rect_max, GetColorU32(ImGuiCol_SliderGrabActive), style.GrabRounding);

    // Add a dot at the end of the slider handle
    ImVec2 dot_pos = ImVec2(grab_center_x, grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y) * 0.5f);
    float dot_radius = 6.0f; // Adjust this value for a larger dot
    window->DrawList->AddCircleFilled(dot_pos, dot_radius, IM_COL32_WHITE);

    // Render text at the center of the slider frame
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    ImVec2 text_size = CalcTextSize(value_buf);
    ImVec2 value_text_pos = ImVec2(frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x - text_size.x) * 0.5f, frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y - text_size.y) * 0.5f);
    window->DrawList->AddText(value_text_pos, GetColorU32(ImGuiCol_Text), value_buf);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + inner_spacing_for_text, frame_bb.Min.y + style.FramePadding.y - 5.0), label);
